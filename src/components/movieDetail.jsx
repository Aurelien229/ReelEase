...
const MovieDetail = () => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${import.meta.env.VITE_API_KEY}&language=fr-FR&append_to_response=credits,videos`);
  ...
  const cast = credits && credits.cast ? credits.cast.slice(0,19) : [];
